    ...
    def get(self, request):
        questions = (
            Question.objects.filter(
                is_deleted=False, status=Question.QuestionStatus.PUBLISHED
            )
            .select_related("author", "job_role")
            .prefetch_related("company", "tag")
        )
...
class QuestionDetailView(APIView):
    ...
    def get(self, request, id):
        question = get_object_or_404(
            Question.objects.select_related("author", "job_role").prefetch_related(
                "company", "tag"
            ),
            id=id,
            is_deleted=False,
        ...
        return Response(
            {"message": "Question hasn't been published yet"},
    ...
    def patch(self, request, id):
        ...
        if request.user.id != question.author.id:
            return Response(
                {"message": "You are not author of the post"},
    ...
    def delete(self, request, id):
        ...
        if question.is_deleted == True:
            return Response(
                {"message": "This post has already deleted."},
                status=status.HTTP_404_NOT_FOUND,
            )
...
class CompanyWiseQuestionView(APIView):
    ...
    def get(self, request, company_id):
        ...
        if not questions:
            return Response(
                {"message": "No questions found"}, status=status.HTTP_404_NOT_FOUND
            )
...
class QuestionStatusDetailView(APIView):
    ...
    def get(self, request, id):
        ...
        question_status = get_object_or_404(
            UserQuestionStatus.objects.select_related(
                "question", "question__author", "question__job_role"
            ).prefetch_related("question__company", "question__tag"),
            question=question,
            user=request.user,
        )
    ...
    def patch(self, request, id):
        ...
        serializer = UserQuestionStatusSerializer(
            question_status, data=request.data, partial=True
        )
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
    ...
    def delete(self, request, id):
        question = get_object_or_404(Question, id=id, is_deleted=False)
        question_status = get_object_or_404(
            UserQuestionStatus, question=question, user=request.user
        )
...
class UserQuestionStatusListView(APIView):
    ...
    def get(self, request):
        question_status = (
            request.user.questions_progress_status.filter(
                question__status=Question.QuestionStatus.PUBLISHED,
                question__is_deleted=False,
            )
            .exclude(status=UserQuestionStatus.StatusChoices.UNSOLVED)
            .select_related("question", "question__author", "question__job_role")
            .prefetch_related("question__company", "question__tag")
        )
